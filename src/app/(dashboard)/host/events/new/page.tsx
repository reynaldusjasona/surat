...
const plans: Plan[] = [
  {
    ...
    label: "Free",
    price: "Free",
    features: ["50 guests", "50 photos", "Basic features"],
  },
  {
    value: "standard",
    label: "Standard",
    price: "SGD 19",
    features: ["200 guests", "500 photos", "All features", "CSV export"],
  },
  {
    value: "premium",
    label: "Premium",
    price: "SGD 49",
    features: ["Unlimited guests", "Unlimited photos", "All features", "Priority support", "Analytics"],
    highlight: true,
  },
];
...
export default function CreateEventPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/dashboard/host/events/${slug}`);
    } catch {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="card p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {eventTypes.map((t) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center gap-1 py-3 rounded-lg border text-sm transition-all",
                    form.type === t.value
                      ? "border-surat-red-400 bg-surat-red-50 text-surat-red-700"
                      : "border-surat-neutral-200 bg-white text-surat-neutral-600 hover:border-surat-beige-400"
                  )}
                >
          ...
          <div>
            ...
            <input
              className="input"
              placeholder="The Ritz-Carlton, Millenia Singapore"
              value={form.location}
            ...
            {plans.map((p) => (
              <button
                ...
                className={cn(
                  ...
                  form.plan === p.value
                    ? "border-surat-red-400 bg-surat-red-50"
                    : "border-surat-neutral-200 bg-white hover:border-surat-beige-400",
                  p.highlight && form.plan !== p.value && "ring-1 ring-surat-red-200"
                )}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold text-sm text-surat-neutral-800">{p.label}</span>
                  {p.highlight && <span className="badge badge-red text-xs">Popular</span>}
                </div>
                ...
                <ul className="space-y-1">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-center gap-1.5 text-xs text-surat-neutral-500">
                      <Check size={11} className="text-surat-success shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
              ...
              <div key={f.key} className="flex items-center justify-between py-2">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    "h-[22px] focus-visible:ring-2 focus-visible:ring-surat-red-300 focus-visible:outline-none",
                    form[f.key] ? "bg-surat-red-500" : "bg-surat-neutral-200"
                  )}
                >
                  <span
                    className={cn(
                      "absolute top-0.5 left-0.5 w-[18px] h-[18px] rounded-full bg-white shadow-sm transition-transform",
                      form[f.key] ? "translate-x-[18px]" : "translate-x-0"
                    )}
                  />
                </button>
              </div>
