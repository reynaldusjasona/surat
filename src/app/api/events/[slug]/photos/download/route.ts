...
export async function GET(
  ...
  try {
    ...
    // Check if unlocked
    const unlock = await prisma.photoUnlock.findUnique({
      where: {
        eventId_guestEmail: {
          eventId: event.id,
          guestEmail: email,
    ...
    const signedPhotos = await Promise.all(
      photosToReturn.map(async (photo) => {
        ...
        let signedUrl = photo.fileUrl; // fallback to public URL
    ...
    return NextResponse.json({
      data: {
        ...
        returnedPhotos: photosToReturn.length,
      },
      error: null,
    });
  } catch (error) {
